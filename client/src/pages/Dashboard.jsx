...
const Dashboard = () => {
  ...
  //   try {
  //     // const response = await axios.get("/api/health-metrics/weight");
  //     setWeightHistory(userData.weight);
  ...
  //   try {
  //     const response = await axios.get("/api/health-metrics/bmi");
  ...
  //     renderBmiChart();
  //   }
  // }, [weightHistory, bmiHistory]);

  // Calculate BMI whenever weight or height changes
  useEffect(() => {
    if (userData.weight[userData.weight.length-1] && userData.height[userData.height.length-1]) {
      const heightInMeters = userData.height[userData.height.length-1] / 100;
      const bmi = userData.weight[userData.weight.length-1] / (heightInMeters * heightInMeters);
      setUserData((prev) => ({
        ...prev,
        bmi: Number.parseFloat(bmi.toFixed(1)),
      }));
  ...
  // const renderWeightChart = () => {
  //   if (!weightChartRef.current || weightHistory.length === 0) return;
  ...
  //     const label = document.createElement("div");
  //     label.className =
  //       "text-xs text-gray-500 dark:text-gray-400 mt-1 text-center";
  //     label.textContent = new Date(data.date).toLocaleDateString("en-US", {
  //       month: "short",
  //     });
  ...
  // const renderBmiChart = () => {
  //   if (!bmiChartRef.current || bmiHistory.length === 0) return;

  //   const container = bmiChartRef.current;
  //   container.innerHTML = "";

  //   // ... rest of your existing renderBmiChart implementation ...
  //   // (keep the same code, just make sure it uses bmiHistory from state)
  //   const rangesContainer = document.createElement("div");
  //   rangesContainer.className = "absolute inset-0 flex flex-col";
  ...
  //   const overweightRange = document.createElement("div");
  //   overweightRange.className =
  //     "flex-1 bg-yellow-50 dark:bg-yellow-900/20 opacity-30";

  //   const normalRange = document.createElement("div");
  //   normalRange.className =
  //     "flex-1 bg-green-50 dark:bg-green-900/20 opacity-30";

  //   const underweightRange = document.createElement("div");
  //   underweightRange.className =
  //     "flex-1 bg-blue-50 dark:bg-blue-900/20 opacity-30";
  ...
  //   const obeseLabel = document.createElement("div");
  //   obeseLabel.className =
  //     "border-b border-red-200 dark:border-red-700 flex justify-end";
  //   obeseLabel.innerHTML =
  //     '<span class="text-xs text-red-500 dark:text-red-400 pr-1">30 - Obese</span>';

  //   const overweightLabel = document.createElement("div");
  //   overweightLabel.className =
  //     "border-b border-yellow-200 dark:border-yellow-700 flex justify-end";
  //   overweightLabel.innerHTML =
  //     '<span class="text-xs text-yellow-500 dark:text-yellow-400 pr-1">25 - Overweight</span>';

  //   const normalLabel = document.createElement("div");
  //   normalLabel.className =
  //     "border-b border-green-200 dark:border-green-700 flex justify-end";
  //   normalLabel.innerHTML =
  //     '<span class="text-xs text-green-500 dark:text-green-400 pr-1">18.5 - Normal</span>';

  //   const underweightLabel = document.createElement("div");
  //   underweightLabel.className =
  //     "border-b border-blue-200 dark:border-blue-700 flex justify-end";
  //   underweightLabel.innerHTML =
  //     '<span class="text-xs text-blue-500 dark:text-blue-400 pr-1">Underweight</span>';
  ...
  const saveProfileChanges = async () => {
    ...
    try {
      const response = await axios.patch(`${backendUrl}/api/user/update-basic`, {
        name: editData.name,
        age: newAge,
        gender: editData.gender,
        height: newHeight,
      ...
      // Refresh weight and BMI data after update
      // fetchWeightHistory();
      // fetchBmiHistory();
      fetchUserProfile();
  ...
  // (getBmiCategory, fadeInUp, and the JSX return)
  const getBmiCategory = (bmi) => {
    if (bmi < 18.5)
      return {
  ...
  const bmiStatus = getBmiCategory(userData.bmi);
  console.log("BMI Status:", userData.bmi);
  ...
  return (
    ...
    // Just make sure to add loading states where appropriate
    // <div className="health-container py-8">
    //   {loading.profile || loading.medical || loading.metrics ? (
      ...
      // ) : (
        ...
        <div className="health-container py-8">
          ...
          >
            ...
            {/* Health Metrics */}
            <motion.div
              className="card col-span-1 md:col-span-3"
              variants={fadeInUp}
            >
              <div className="p-6">
                <h2 className="text-xl font-semibold flex items-center mb-6 text-gray-800 dark:text-gray-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="border border-gray-200 dark:border-gray-700 rounded-lg">
                    ...
                    <div className="p-4 h-64 relative">
                      <div
                        ref={weightChartRef}
                        className="chart-container flex items-end justify-between"
                      ></div>
                    </div>
                  </div>
